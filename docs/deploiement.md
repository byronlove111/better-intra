...
| `JWT_SECRET` | Clé de signature des JWT. **Obligatoire** (env uniquement, pas de défaut dans le code). Générer avec `openssl rand -base64 48`. Vide → `compose` et le backend refusent de démarrer. Algo / durées restent dans `app/config.py`. |
...
Copier `.env.example` en `.env` ne suffit **pas** à lancer la stack tel quel : `JWT_SECRET`, `FORTY_TWO_CLIENT_ID` et `FORTY_TWO_CLIENT_SECRET` y sont vides par design (secrets, jamais commités), et chaque variable requise fait crasher `docker compose up` si elle est absente ou vide (`${VAR:?manquant dans .env}`, voir tableau ci-dessus). Il faut au minimum un `JWT_SECRET` généré et tes creds OAuth 42 avant `make up`.
